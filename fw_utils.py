...
def pseudo_beam_search_batch(
    ...
    while len(seen) < s and total_requests < max_total_requests:
        ...
        try:
            ...
            response = requests.post(url, headers=headers, data=json.dumps(payload), timeout=REQUEST_TIMEOUT_SEC)
...
def get_next_token_dist(prompt, model, url, headers, logprobs_count=LOGPROBS_COUNT):
...
def main():
    ...
    next_token_dist = get_next_token_dist(
        prompt=PROBLEM_PROMPT,
        model=model,
        url=url,
        headers=headers,
    )
    pprint.pprint(next_token_dist)
